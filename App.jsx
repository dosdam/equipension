...
export default function App() {
  ...
  const printOutingReport = (e) => {
    ...
    if (reportStartDate > reportEndDate) {
      notify("La date de début doit précéder la date de fin");
      return;
    }
    const outings = (Array.isArray(selected.outings) ? selected.outings : [])
      .filter(
        (outing) =>
          String(outing.date || "") >= reportStartDate &&
          String(outing.date || "") <= reportEndDate,
      )
    ...
    const rows = outings.length
      ? outings
          .map(
            (outing) => `
              <tr>
                <td>${escapeHtml(fmt(outing.date))}</td>
                <td>${escapeHtml(outing.place || "Non renseignée")}</td>
                <td>${escapeHtml(outing.duration || "Non renseignée")}</td>
                <td>${escapeHtml(outing.by || "Non renseigné")}</td>
              </tr>`,
          )
          .join("")
      : '<tr><td colspan="4" class="empty">Aucune sortie sur cette période.</td></tr>';
    const reportWindow = window.open("", "_blank");
    if (!reportWindow) {
      notify("Autorisez les fenêtres pop-up pour éditer le PDF");
      return;
    }
    reportWindow.document.write(`<!doctype html>
      <html lang="fr">
        ...
        <body>
          <h1>Récapitulatif des sorties</h1>
          <div class="period"><strong>Équidé :</strong> ${escapeHtml(selected.name)}<br />
            <strong>Du :</strong> ${escapeHtml(fmt(reportStartDate))}
            <strong> au :</strong> ${escapeHtml(fmt(reportEndDate))}</div>
          <table>
            <thead><tr><th>Date</th><th>Destination</th><th>Durée</th><th>Déclarée par</th></tr></thead>
            <tbody>${rows}</tbody>
          </table>
        </body>
      </html>`);
        ...
        {modal === "outingReport" && (
          <Modal
            ...
            <form onSubmit={printOutingReport} className="space-y-4">
              <p className="text-sm text-slate-600">
                Le document contiendra les sorties de {selected.name} entre les
                deux dates choisies.
              </p>
